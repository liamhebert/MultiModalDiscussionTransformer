...
def contrastive_collator(items, spatial_pos_max=10):
    ...
    # normalized weight
    extra_weight = (torch.logical_or(y_matrix.eq(1), hard_y_matrix.eq(1))).sum(
        dim=1
    ) / (torch.logical_and(y_matrix.eq(0), hard_y_matrix.eq(0))).sum(
        dim=1
    ...
    soft_matrix = torch.where(
        torch.logical_and(y_matrix.eq(0), hard_y_matrix.eq(0)), extra_weight, 1
    )
    soft_matrix = torch.where(
        torch.eye(soft_matrix.size(0)).eq(1), 0, soft_matrix
    )  # we dont include itself in the loss
    ...
    for key in ["input_ids", "token_type_ids", "attention_mask"]:
        x[key] = torch.cat(
            [pad_2d_custom_unsqueeze(a[key], max_node_num, 0) for a in xs]
        )
    ...
    x_image_indexes = torch.cat(
        [pad_1d_custom_unsqueeze(z, -1, False).squeeze(0) for z in x_image_indexes]
    ).bool()
...
def node_collator(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=10):
    items = [item for item in items if item is not None]
    items = [
        (
            item.idx,
