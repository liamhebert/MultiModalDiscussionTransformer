...
class GraphFusionLayer(nn.Module):
    ...
    def forward(self,         
        ...
        bert_hidden_output_out = self.bert_forward(bert_hidden_states_in, 
                          bert_attention_mask, 
                          None, 
                          None, 
                          None)
        ...
        if vit_hidden_states != None:
            vit_hidden_states_in = torch.cat([bottle_neck[x_image_indexes], vit_hidden_states], dim=1)
            ...
            bottle_neck_output[x_image_indexes] = (vit_hidden_output_out[:, :self.num_bottle_neck_tokens] + bottle_neck_output[x_image_indexes]) / 2
...
class GraphFusionStack(nn.Module):
    def __init__(self, bert_layers, vit_layers, num_bottle_neck_tokens, use_projection = False) -> None:
        super().__init__()

        self.fusion_layers = nn.ModuleList([
            GraphFusionLayer(bert_layer, vit_layer, num_bottle_neck_tokens, use_projection) for bert_layer, vit_layer in zip(bert_layers, vit_layers)
        ])
    ...
    def forward(self,
        ...
        for f_layer in self.fusion_layers:
            bert_hidden_states, vit_hidden_states, bottle_neck = f_layer(bert_hidden_states, vit_hidden_states, bottle_neck, bert_attention_mask, x_image_indexes)
