...
@register_criterion("node_cross_entropy_no_filter", dataclass=FairseqDataclass)
class GraphPredictionNodeCrossEntropy(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        ncorrect = (torch.argmax(logits, dim=-1) == targets).sum()
        #print(targets)
...
@register_criterion("node_binary_cross_entropy_no_filter_two", dataclass=FairseqDataclass)
class GraphPredictionBinaryNodeCrossEntropy(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logits = model(**sample["net_input"])[:, 1:, :]
        #targets = model.get_targets(sample, [logits])[: logits.size(0)]
        targets = sample["net_input"]["batched_data"]["y"]
        target_mask = sample["net_input"]['batched_data']['y_mask']
        ...
        ncorrect = ((logits > 0.5) == targets).sum()
        #print(targets)
        loss = self.loss(
            logits, targets
        )
