...
from ..modules import init_graphormer_params, NoGraphormerGraphEncoder
...
@register_model("no_graphormer")
class NoGraphormerModel(FairseqEncoderModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = NoGraphormerEncoder(args)
...
class NoGraphormerEncoder(FairseqEncoder):
    def __init__(self, args):
        super().__init__(dictionary=None)
        self.max_nodes = args.max_nodes

        self.graph_encoder = NoGraphormerGraphEncoder(
            # < for graphormer
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **unused):
        bert_cls, vit_cls = self.graph_encoder(
            batched_data,
            perturb=perturb,
        )
        ...
        bert_cls = self.layer_norm(self.activation_fn(self.lm_head_transform_weight(bert_cls)))

        # project back to size of vocabulary
        if self.share_input_output_embed and hasattr(
            self.graph_encoder.embed_tokens, "weight"
        ):
            bert_cls = F.linear(bert_cls, self.graph_encoder.embed_tokens.weight)
        elif self.embed_out is not None:
            bert_cls = self.embed_out(bert_cls)
        if self.lm_output_learned_bias is not None:
            bert_cls = bert_cls + self.lm_output_learned_bias

        if vit_cls != None:
            vit_cls = self.layer_norm(self.activation_fn(self.lm_head_transform_weight(vit_cls)))

            # project back to size of vocabulary
            if self.share_input_output_embed and hasattr(
                self.graph_encoder.embed_tokens, "weight"
            ):
                vit_cls = F.linear(vit_cls, self.graph_encoder.embed_tokens.weight)
            elif self.embed_out is not None:
                vit_cls = self.embed_out(vit_cls)
            if self.lm_output_learned_bias is not None:
                vit_cls = vit_cls + self.lm_output_learned_bias

            bert_cls[batched_data['x_image_indexes']] = (vit_cls + bert_cls[batched_data['x_image_indexes']]) / 2
        return bert_cls
...
@register_model_architecture("no_graphormer", "no_graphormer")
...
@register_model_architecture("no_graphormer", "no_graphormer_base")
...
@register_model_architecture("no_graphormer", "no_graphormer_slim")
...
@register_model_architecture("no_graphormer", "no_graphormer_large")
