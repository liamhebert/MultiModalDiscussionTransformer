...
class MultiGraphormerGraphEncoderCLIP(nn.Module):
    def __init__(
        self,
        num_atoms: int,
        num_in_degree: int,
    ...
    def build_vit_bert_encoders(self, num_fusion_layers):
        vit_model = CLIPVisionModel.from_pretrained("openai/clip-vit-base-patch32")
        ...
        if num_fusion_layers == 0:
            ...
            vit_other_layers = vit_model.vision_model.encoder.layers[-num_fusion_layers:]
            vit_model.vision_model.encoder.layers = vit_model.vision_model.encoder.layers[:-num_fusion_layers]
    ...
    def forward(
        ...
        for g_layer, \
            ...
            g_norm, \
            g_to_b_projection, \
            g_to_v_projection in zip(self.layers, \
                                     self.fusion_layers, \
                                     self.graph_norm_layers, \
                                     self.graph_to_bert_projection_layers, \
                                     self.graph_to_vit_projection_layers):
